...
if (isset($_POST['submit'])) {
    ...
    $sqlInsert = "INSERT INTO tblpayment (fullName, orderID, email, address, city, state, zip, name_OC, cc_num, exp_month, exp_year, cvv, totalprice)
        VALUES ('$fname','$orderId', '$email', '$add', '$city', '$state', '$zip', '$cname', '$cardNum', '$expmonth', '$expyear','$cvv', '$totalprice')";
    mysqli_query($conn, $sqlInsert);
    
    // Update tblorder with paymentId and status
    $sqlUpdate = "UPDATE tblorder SET paymentId = LAST_INSERT_ID(), status = 'APPROVED' WHERE orderId = '$orderId'";
    mysqli_query($conn, $sqlUpdate);

    mysqli_close($conn);
    echo '<script>
          alert("Payment Approved u!!");
  ...
  <div class="col-75">
    <div class="container">
      <form method="post">
        ...
        <input type="submit" name="submit" value="PROCEED TO CHECKOUT" class="btn">
      </form>
    </div>
  </div>
</div>
